...
class DaViT(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=1000, depths=(1, 1, 3, 1), patch_size=4,
                 ...
                 img_size=224, head_embed_dim = 512, drop_rate=0., attn_drop_rate=0., **kwargs
                 ):
        super().__init__()
        ...
        for block_index, block_param in enumerate(self.architecture):
            ...
            for branch_id in branch_ids:
                if branch_id not in branches:
                    x, size = self.patch_embeds[branch_id](features_list[-1], sizes[-1])
...
def DaViT_tiny(pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay=None, **kwargs):
    model_kwargs = dict(
        patch_size=2, window_size=7, embed_dims=(96, 192, 384, 768), num_heads=(3, 6, 12, 24),
...
def DaViT_base(pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay=None, **kwargs):
    model_kwargs = dict(
        patch_size=2, window_size=7, embed_dims=(128, 256, 512, 1024), num_heads=(4, 8, 16, 32),
        depths=(1, 1, 9, 1), mlp_ratio=4., overlapped_patch=False, **kwargs)
    return DaViT(**model_kwargs)
